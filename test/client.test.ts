...
describe("Btt client", () => {
  it("triggerNamed passes name and wait flag", async () => {
    ...
    expect(t.last()).toEqual({
      ...
      params: { trigger_name: "Foo", wait_for_reply: true },
    });
  });

  it("variables round-trip and infer types", async () => {
    const { t, btt } = make({
      get_number_variable: "0.5",
      get_variable_type: "number",
      get_string_variable: "x",
    });
  ...
  it("triggerAction merges sequences into one action", async () => {
    ...
    const json = t.last().params?.json as {
      BTTPredefinedActionType: number;
      BTTAdditionalActions: unknown[];
    };
  ...
  it("chain.run sends one combined action", async () => {
    ...
    const json = t.last().params?.json as {
      BTTAdditionalActions: Array<{ BTTPredefinedActionType: number }>;
    };
